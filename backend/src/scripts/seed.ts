...
const randBetween = (min: number, max: number) => Math.round(min + Math.random() * (max - min));
const randRating = (min: number, max: number) => +(min + Math.random() * (max - min)).toFixed(1);

/* ───────────────────────────────────────────────────────
   LOCATION DATABASE — 15 India + 15 International
   ─────────────────────────────────────────────────────── */

interface City { name: string; state: string; country: string; lat: number; lng: number; zip: string; }
...
const makeAddress = (city: City, idx: number) => {
...
/* ───────────────────────────────────────────────────────
   LISTING DEFINITIONS — 155 TOTAL
   ─────────────────────────────────────────────────────── */

interface ListingDef {
  name: string;
  type: 'hotel' | 'coworking' | 'nap_pod' | 'lounge' | 'capsule_hotel' | 'meeting_room' | 'transit_room';
  cityIdx: number; // index into ALL_CITIES
  priceMin: number;
  priceMax: number;
  desc: string;
}
...
const inCity = (i: number) => i % INDIA_CITIES.length;
const intlCity = (i: number) => INDIA_CITIES.length + (i % INTL_CITIES.length);

const HOTELS: ListingDef[] = [
  // India Hotels (35)
...
const MEETING_ROOMS: ListingDef[] = [
...
const getImages = (type: string, count: number): string[] => {
  const pool = IMAGE_POOLS[type] || HOTEL_IMAGES;
  if (!imageCounters[type]) imageCounters[type] = 0;
  const images: string[] = [];
...
const getAmenities = (type: string): string[] => {
...
const ROOM_TYPES_BY_PROP: Record<string, { type: string; label: string; capacity: number; sizeSqft: number }[]> = {
  hotel: [
...
const seed = async () => {
  ...
  logger.info('🌱 Starting RESTIGO production seed (155 listings)...');
  ...
  logger.info('  ✓ Created 12 users (1 admin, 10 providers, 1 test user)');
  ...
  for (let i = 0; i < ALL_LISTINGS.length; i++) {
    ...
    try {
      ...
      try {
        ...
        logger.warn(`  ⚠ Skipped duplicate: ${listing.name} — ${err2.message}`);
      }
    }
  }
  logger.info(`  ✓ Created ${created} properties`);
  ...
  for (const property of properties) {
    ...
    await Property.findByIdAndUpdate(property._id, { totalRooms: roomCount });
  }
  ...
  for (let i = 0; i < slotsToInsert.length; i += chunkSize) {
    ...
    try {
      await InventorySlot.insertMany(chunk, { ordered: false });
      totalSlots += chunk.length;
      logger.info(`  ✓ Inserted batch of ${chunk.length} inventory slots`);
    } catch (err: any) {
      if (err.insertedDocs) {
  ...
  logger.info(`  ✓ Created ${totalRooms} rooms`);
  logger.info(`  ✓ Created ${totalSlots} inventory slots`);
  ...
  const stats = {
    ...
    slots: await InventorySlot.countDocuments(),
  };

  logger.info('');
  logger.info('═══════════════════════════════════════════');
  logger.info('  ✅ RESTIGO Database Seeded Successfully');
  logger.info('═══════════════════════════════════════════');
  logger.info(`  Users:       ${stats.users}`);
  logger.info(`  Properties:  ${stats.properties}`);
  logger.info(`  Rooms:       ${stats.rooms}`);
  logger.info(`  Slots:       ${stats.slots}`);
  logger.info('');
  logger.info('  Login credentials:');
  logger.info('    Admin:     admin@restigo.app / Admin@123456');
  logger.info('    Provider:  provider1@restigo.app / Provider@123');
  logger.info('    User:      user@restigo.app / User@123456');
  logger.info('═══════════════════════════════════════════');
