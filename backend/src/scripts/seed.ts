...
const PROPERTY_TYPES = ['hotel', 'transit_room', 'coworking', 'nap_pod', 'lounge', 'capsule_hotel', 'meeting_room', 'short_stay_apartment'] as const;
const ROOM_TYPES = ['standard', 'deluxe', 'suite', 'pod', 'capsule', 'desk', 'meeting_room', 'private_office'] as const;
...
const seed = async () => {
  ...
  await Promise.all([
    ...
    Room.deleteMany({}), InventorySlot.deleteMany({}),
  ]);
  ...
  logger.info(stats, '✅ Database seeded successfully');
